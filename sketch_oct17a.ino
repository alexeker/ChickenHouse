...
iarduino_RTC time(RTC_DS1302, RST, CLK, DAT); // подключаем RTC модуль на базе чипа DS1302, указывая выводы Arduino подключённые к выводам модуля RST, CLK, DAT
...
void setup() {
  ...
  pinMode(sensor, INPUT);
   Serial.begin(9600);
    time.begin();
   //time.settime(0,24,23,17,10,20,6);  // 0  сек, 51 мин, 21 час, 27, октября, 2015 года, вторник
...
if(millis()%1000==0){ // если прошла 1 секунда
      ...
      nowHour = atol(time.gettime("H")); // atol() функция преобразования char в int
      Serial.println(sensorValue);
      delay(1); // приостанавливаем на 1 мс, чтоб не выводить время несколько раз за 1мс
     Serial.println(nowHour >= startHour);
      Serial.println(nowHour  <= stopHour);
      if(nowHour >= startHour && nowHour <= stopHour && sensorValue <= porog){
        digitalWrite(relay, HIGH);
        Serial.println("Usloviya true");
      }else {digitalWrite(relay,LOW); Serial.println("Usloviye ne true");}
         //Функция отката на 5 сек каждые сутки
if(millis()%86400000==0){
  ...
  int hour;
  int newhours;
  secs = atol(time.gettime("s"));
  mins = atol(time.gettime("m"));
  if(secs >= 5){
    newsecs = secs - 5;
    time.settime(newsecs);
  }else{
    newsecs = 60 - secs;
    if(mins != 1){
    newmins = mins -1;
    time.settime(newsecs,newmins);
    }else{
      hour = atol(time.gettime("H"));
      newhours = hour - 1;
      time.settime(newsecs,newmins,newhours);
       }
     }
  }
    }
